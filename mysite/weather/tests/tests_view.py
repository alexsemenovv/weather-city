...
class CityFormTestCase(TestCase):
    ...
    def test_city_form_not_valid(self):
        ...
        response = self.client.post(
            reverse("weather:search_city"), {"city": "Mo66576scow"}
        )
        self.assertEqual(
            response.context.get("form").errors.get("city")[0],
            "Локация должна содержать только буквы",
        )
    ...
    def test_city_form_invalid_city_adds_error(self, mock_prepare_weather_data):
        ...
        response = self.client.post(
            reverse("weather:search_city"), {"city": "вамывамдтфывс"}
        )
